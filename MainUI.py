...
class Script:
    ...
    def create_btn(self):
        cmds.button(label=nice_script_name(self.script_name), command=self.run_script)
    ...
    def create_btn_with_submenu(self):
        cmds.button(label=f'  {nice_script_name(self.script_name)} >', command=self.run_script)
...
def create_panels():
    def create_display_panel(type='display'):
        ...
        Script(type, 'ProxyAttrs').create_btn()
    ...
    def create_modify_panel(type='modify'):
        ...
        Script(type, 'RotateOrder').create_btn()
        Script(type, 'BatchMatchTransform').create_btn_with_submenu()
        Script(type, 'MatchPosBetween').create_btn()
        Script(type, 'MatchRotBetween').create_btn()
        Script(type, 'MatchToVertex').create_btn()
        Script(type, 'MatchRotateAxis').create_btn()
        Script(type, 'IndividuallyFreeze').create_btn()
    ...
    def create_rigging_panel(type='rigging'):
        ...
        Script(type, 'CheckUnfrozenJoints').create_btn()

        cmds.text(l='Create')
        Script(type, 'CreateRoundedSquare').create_btn()
        Script(type, 'BatchCreateControl').create_btn()
        Script(type, 'BatchCreateOffsetGroup').create_btn()
        Script(type, 'BatchCreateDrivenGroup').create_btn()
        Script(type, 'BatchCreateConstriant').create_btn_with_submenu()
        # Script(type, 'BatchControlConstraintOffset').create_btn()

        cmds.text(l='Attribute')
        Script(type, 'AddAttributeSeparator').create_btn()
        Script(type, 'AddFootAttributes').create_btn()
        Script(type, 'AttributeConnector').create_btn_with_submenu()

        cmds.text(l='Automation')
        Script(type, 'AutoMirror').create_btn_with_submenu()

        cmds.text(l='Matrix')
        Script(type, 'TransferToOfstParentMatrix').create_btn()
        Script(type, 'UpdateOfstParentMatrix').create_btn()
